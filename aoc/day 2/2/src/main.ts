...
function getSubArray(array: number[], index: number): number[] {
    array.splice(index, 1);
    return array;
}

function getAllSubArray(array: number[]) {

    const out: number[][] = [];

    for (let i = 0; i < array.length; i++) {
        out[i] = getSubArray([...array], i)
    }
...
async function main() {
    ...
    rows.forEach((row, index) => {
        ...
        const array = stringToNumberArray(row)

        if (validateArray(array)) out++;
        else {

            const allSubs = getAllSubArray(array);

            if(allSubs.some(v => validateArray(v))){
                out++;
            }
